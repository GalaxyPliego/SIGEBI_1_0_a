          ...
          <table class="table table-striped tableBecas">
            ...
            <c:forEach items="${listaBecas}" var="beca" varStatus="status">
              <tr>
                ...
                <td>${beca.estado}</td>
                <td>
                  <form action="ServletBecasRU" method="post">
                    <input type="hidden" name="idBeca" value="${beca.idBeca}">
                    <input type="hidden" name="opc" value="R">
                    <button class="btn btn-info"><i class="fas fa-angle-double-right"></i> Modificar</button>
                  </form>
                </td>
              </tr>
            </c:forEach>
      ...
      <div class="col-12">
        <form action="ServletBecaRU" method="post">
          ...
          <div class="form-group">
            <label for="tipoBeca">Tipo Beca</label>
            <input type="text" class="form-control" name="tipoBeca" id="tipoBeca" value="${beca.tipoBeca}">
          </div>

          <div class="form-group">
            <label for="estadoBeca">Estado</label>
            <select name="estadoBeca" class="form-control" id="estadoBeca" required>
              <option value="${beca.estado} selected">${beca.estado}</option>
          ...
          <div class="form-group">
            <button type="submit" class="btn btn-success btn-block btn-">Modificar</button>
          </div>
        </form>
      </div>
    </div>
